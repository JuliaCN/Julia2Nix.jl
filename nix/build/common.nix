{
  ...
  packages = callPackage args.packages {};
  ### Repoify packages
  ...
  repoified = map (item:
    if item.src == null
    then item
    else item // {src = repoify item.name item.treehash item.src;})
  packages.closure;
  ...
  manifestToml = runCommand "Manifest.toml" {buildInputs = [jq];} ''
    cp ${args.importManifest} ./Manifest.toml
  ...
  depot =
    runCommand "julia-depot" {
      ...
      cp ${args.importProject} ./Project.toml
