...
let
  ...
  selectFetcher = lib.flatten (
    (attr: (map (name: (lib.optionals (lib.hasAttr name attr) name)) fetches))
    ...
    let
      concatAttrs =
        let
          f =
            x:
            lib.foldr (n: acc: acc // lib.mapAttrs (_: v: v) (x.${n})) { } (
              lib.attrNames x
            );
        in
        f;
    ...
    concatAttrs (
      lib.listToAttrs (
        map
          (name: {
            inherit name;
            value = lib.mapAttrs (_: (lib.getAttrFromPath [ name ]) (pkgs // builtins)) (
              (lib.recursiveUpdate (lib.importTOML julia2nix).depot.${stdenvNoCC.system}
                extraJulia2nix
              )."${name}"
            );
          })
        ...
        stdenvNoCC.mkDerivation {
          ...
          installPhase =
            ''
              ...
              runHook postInstall
            ''
            + lib.optionalString patch-context (
              let
